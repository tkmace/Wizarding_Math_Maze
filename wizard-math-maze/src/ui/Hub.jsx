import { OPS, DIFFS } from '../game/math.js'
import { skinById, getSkin, getNext, rankProgress } from '../game/skins.js'
import { C, sans, serif, btn, panel, label } from './theme.js'

/** The castle: pick what to practise, see your rank, head into a maze. */
export default function Hub({ profile, ops, diff, onToggleOp, onSetDiff, onStart, onWardrobe, onReport, onScroll, onLogout, newSkin }) {
  const skin = skinById(profile.equippedSkin)
  const rank = getSkin(profile.totalPoints)
  const next = getNext(profile.totalPoints)
  ...
  return (
    <div className="appear scroll" style={{ zIndex: 10, width: '100%', maxWidth: 470, padding: '8px 14px 24px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 14 }}>
        <div className="wf" style={{ fontSize: 92, lineHeight: 1, filter: `drop-shadow(0 0 26px ${skin.color})` }}>{skin.emoji}</div>
        <div style={{ color: '#fff', fontWeight: 900, fontSize: 22, fontFamily: sans, marginTop: 2 }}>{profile.name}</div>
        <div style={{ color: rank.color, fontFamily: serif, fontSize: 12, letterSpacing: 2, fontWeight: 900 }}>
          {rank.wand} {rank.title.toUpperCase()}
        </div>
      </div>
      ...
      <div style={panel({ padding: '14px 16px', marginBottom: 12 })}>
        ...
        <div style={{ height: 9, background: '#0a0a2c', borderRadius: 9, overflow: 'hidden', border: `1px solid ${C.line}` }}>
          <div style={{
            ...
            background: `linear-gradient(90deg,${rank.color},${next ? next.color : C.goldHi})`,
            boxShadow: `0 0 12px ${rank.color}99`, transition: 'width .5s ease-out',
          }} />
        </div>
        <div style={{ color: C.faint, fontSize: 10, marginTop: 6, letterSpacing: 0.5 }}>
          {next
            ? `${(next.threshold - profile.totalPoints).toLocaleString()} points to ${next.title} ${next.emoji}`
            : 'Every rank unlocked — you are legend. ☄️'}
        </div>
      </div>
          ...
          {DIFFS.map(d => {
            ...
            return (
              <button key={d.key} className="bh" onClick={() => onSetDiff(d.key)} style={{
                ...
                <span style={{ color: d.color, fontSize: 10, fontWeight: 900 }}>×{d.mult}</span>
              </button>
            )
          })}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8, marginTop: 10 }}>
        <button className="bh" onClick={onWardrobe} style={btn('ghost', { fontSize: 13, position: 'relative' })}>
          ...
          {newSkin && <span style={{
            ...
            background: C.bad, border: '2px solid ' + C.bg,
          }} />}
        </button>
