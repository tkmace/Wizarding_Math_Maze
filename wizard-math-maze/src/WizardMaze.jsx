...
function genMaze(ops,dk){
  ...
  if(path){
    ...
    for(const[r,c]of pathPlain){if(needed<=0)break;g[r][c]=DOOR;dq[`${r},${c}`]=genQ(ops,dk);needed--}
  }
  return{grid:g,dq}
}
...
export default function WizardMaze(){
  ...
  // start/finish label positions in px
  const startLabelLeft=(1*(C+2))+C/2
  const endLabelLeft=((cols-2)*(C+2))+C/2
      ...
      {screen==='login'&&(
        <div className="appear" style={{textAlign:'center',zIndex:10,maxWidth:420,width:'100%',padding:'0 16px'}}>
          ...
          <div className={loginShake?'shake':''} style={{background:'#0e0e35',border:`2px solid ${loginError?'#ff5555':'#252558'}`,borderRadius:20,padding:'28px 24px',transition:'border-color .2s'}}>
            ...
            <label style={{display:'block',color:'#c8a4ff',fontFamily:"'Cinzel',serif",fontSize:10,letterSpacing:2,marginBottom:7,textAlign:'left'}}>SECRET PASSCODE <span style={{color:'#404070',fontSize:9,letterSpacing:1}}>(4–6 numbers)</span></label>
      ...
      {screen==='game'&&gameScreen==='game'&&maze&&(
        <div style={{zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',gap:7}}>
          ...
          <div style={{position:'relative'}}>
            ...
            {/* START label — top-left above the maze */}
            <div style={{position:'absolute',top:-18,left:7+(1*(C+2)),transform:'translateX(-50%) translateX('+(C/2)+'px)',fontFamily:"'Cinzel',serif",fontSize:9,fontWeight:900,color:'#7ee8a2',letterSpacing:1,textShadow:'0 0 8px #7ee8a2',whiteSpace:'nowrap',pointerEvents:'none',zIndex:10}}>▼ START</div>

            {/* FINISH label — bottom-right below the maze */}
            <div style={{position:'absolute',bottom:-18,right:7+((cols-1-(cols-2))*(C+2)),transform:'translateX(50%) translateX(-'+(C/2)+'px)',fontFamily:"'Cinzel',serif",fontSize:9,fontWeight:900,color:'#c8a4ff',letterSpacing:1,textShadow:'0 0 8px #c8a4ff',whiteSpace:'nowrap',pointerEvents:'none',zIndex:10}}>FINISH ▲</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:`repeat(${cols},${C}px)`,gridTemplateRows:`repeat(${rows},${C}px)`,gap:2,padding:7,background:'#080820',borderRadius:12,border:'2px solid #1a1a45',boxShadow:'0 0 40px #08082840'}}>
              {grid.map((row,ri)=>row.map((cv,ci)=>{
                ...
                return(<div key={`${ri}-${ci}`} style={{width:C,height:C,borderRadius:4,background:cv===WALL?'linear-gradient(135deg,#0b0b25,#10102c)':'linear-gradient(135deg,#14144a,#18184e)',border:cv===WALL?'1px solid #0d0d26':'1px solid #20204c',display:'flex',alignItems:'center',justifyContent:'center',position:'relative'}}>
                  {cv===DOOR&&dqv&&(
                    <div className={dw?'dw':'dp'} style={{width:C-5,height:C-5,borderRadius:5,background:dw?'linear-gradient(135deg,#2a1000,#3a1800)':'linear-gradient(135deg,#261500,#351e00)',border:`2px solid ${dw?'#ff8855':'#f9ca74'}`,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:1}}>
                      <span style={{fontSize:C>42?15:11}}>🔐</span>
                      {/* ── Brighter, larger point label on doors ── */}
                      <span style={{fontSize:C>42?9:7,fontWeight:900,color:dw?'#ffaa77':'#ffe080',fontFamily:"'Cinzel',serif",lineHeight:1,textShadow:`0 0 6px ${dw?'#ff8855':'#f9ca74'}`}}>+{dqv.curPts}p</span>
                      {dw&&<span style={{fontSize:7,color:'#ffaa77',lineHeight:1}}>{'⚠️'.repeat(Math.min(dqv.wrongs,3))}</span>}
                    </div>
                  )}
        ...
        <div style={{position:'fixed',inset:0,background:'#00000092',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,backdropFilter:'blur(5px)'}}>
          <div className={`appear${wrong?' shake':''}`} style={{background:'linear-gradient(160deg,#0f1048,#171858)',border:`2px solid ${q.wrongs>0?'#ff8855':'#f9ca74'}`,borderRadius:22,padding:'clamp(16px,4vw,32px) clamp(16px,5vw,40px)',textAlign:'center',boxShadow:'0 0 70px #f9ca7438',minWidth:270,maxWidth:'90vw',width:'min(370px,90vw)'}}>
            ...
            <input
              ...
              onKeyDown={e=>{if(e.key==='Enter')submit();if(e.key==='Escape')setShowMath(false)}}
              style={{width:'100%',padding:'12px 13px',fontSize:28,fontWeight:900,background:'#0a0a2c',border:`2px solid ${wrong?'#ff5555':'#4a4ab0'}`,borderRadius:11,color:wrong?'#ff6655':'#ffffff',textAlign:'center',marginBottom:10,outline:'none',fontFamily:"'Nunito',sans-serif",textShadow:wrong?'none':'0 0 10px #9090ff44'}}
              placeholder="?"
            />
